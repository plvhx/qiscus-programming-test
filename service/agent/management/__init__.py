...
class Management(Abstract):
    ...
    def get_all_agents(
        self, page=None, limit=None, search=None, scope=None, headers=None
    ):
    ...
    def get_agents_by_division(
        ...
        if isinstance(is_available, bool):
            query.append(
                "is_available={}".format("true" if is_available == True else "false")
            )
    ...
    def get_user_channel(self, user_id, channel=None, headers=None):
        ...
        if isinstance(channel, str) and (
            channel == c.WHATSAPP
            or channel == c.FACEBOOK
            or channel == c.LINE
            or channel == c.QISCUS
            or channel == c.CUSTOM
            or channel == c.TELEGRAM
        ):
